...
app.get("/auth/google/callback", async (req, res) => {
  ...
  try {
    ...
    res.redirect(
      `${process.env.FRONTEND_URL}?auth=success&tokens=${tokenString}`,
    );
  } catch (error) {
...
app.get("/folders", async (req, res) => {
  try {
    ...
    try {
      ...
      console.log(
        `Found ${allShortcuts.data.files?.length || 0} shortcuts total:`,
    ...
    const queries = [
      ...
      drive.files.list({
        ...
        fields:
          "files(id, name, parents, shortcutDetails, driveId, capabilities, mimeType)",
        includeItemsFromAllDrives: true,
        supportsAllDrives: true,
      }),
      // Dedicated query for folder shortcuts
      drive.files.list({
        q: "mimeType='application/vnd.google-apps.shortcut'",
        fields: "files(id, name, parents, shortcutDetails, mimeType)",
        includeItemsFromAllDrives: true,
        supportsAllDrives: true,
      }),
    ];
    ...
    if (responses[0].data.files) {
      allFolders.push(
        ...responses[0].data.files.map((folder) => ({
          ...folder,
          isShared: false,
          folderType: "owned",
          driveType: "personal",
    ...
    if (responses[1].data.files) {
      allFolders.push(
        ...responses[1].data.files.map((folder) => ({
          ...folder,
          isShared: true,
          folderType: "shared",
          driveType: "personal",
    ...
    if (responses[2].data.files) {
      console.log(
        `Processing ${responses[2].data.files.length} items from broader query`,
      );

      for (const item of responses[2].data.files) {
        console.log(
          `Item: ${item.name}, mimeType: ${item.mimeType}, driveId: ${item.driveId}, hasShortcut: ${!!item.shortcutDetails}`,
        );
        ...
        if (
          ...
          console.log(
            `Adding folder shortcut: ${item.name} (shortcut ID: ${item.id}, target ID: ${item.shortcutDetails.targetId})`,
          );
          allFolders.push({
            id: item.shortcutDetails.targetId || item.id,
            name: item.name || "Unnamed Shortcut",
          ...
          if (!existingFolder) {
            console.log(
              `Adding shared drive folder: ${item.name} (driveId: ${item.driveId})`,
            );
            allFolders.push({
              ...item,
    ...
    // Process dedicated shortcuts query
    if (responses[3].data.files) {
      console.log(
        `Processing ${responses[3].data.files.length} shortcuts from dedicated query`,
      );

      for (const item of responses[3].data.files) {
        // Only process folder shortcuts and avoid duplicates
        if (
          item.shortcutDetails?.targetMimeType ===
          "application/vnd.google-apps.folder"
        ) {
          const existingShortcut = allFolders.find((f) => f.id === item.id);
          if (!existingShortcut) {
            console.log(
              `Adding folder shortcut from dedicated query: ${item.name} (shortcut ID: ${item.id}, target ID: ${item.shortcutDetails.targetId})`,
            );
            allFolders.push({
              id: item.shortcutDetails.targetId || item.id,
              name: item.name || "Unnamed Shortcut",
    ...
    if (sharedDrives.data.drives) {
      let responseIndex = 4; // Skip first 4 responses (owned, shared folders, broader query, and dedicated shortcuts)
    ...
    console.log(`Found ${allFolders.length} folders total:`, {
      ...
      sharedDrive: allFolders.filter((f) => f.folderType === "shared-drive")
        .length,
      sharedDriveShortcuts: allFolders.filter(
        (f) => f.folderType === "shared-drive-shortcut",
      ).length,
    });
...
const downloadFileRecursively = async (
  drive: any,
  fileId: string,
  filePath: string,
  supportsSharedDrives: boolean = false,
): Promise<any> => {
  try {
    console.log(
      `Attempting to download file: ${filePath} (fileId: ${fileId}, supportsSharedDrives: ${supportsSharedDrives})`,
    );
    ...
    if (mimeType === "application/vnd.google-apps.document") {
      ...
      response = await drive.files.export(exportParams, {
        responseType: "stream",
      });
      ...
      response = await drive.files.get(downloadParams, {
        responseType: "stream",
      });
    }
